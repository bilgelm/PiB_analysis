"""
Helper operators for various models
"""
...
def resize_N_N(image, resized=32):
    a, b = image.shape
    return zoom(image, zoom=(resized * 1. / a, resized * 1. / b))
...
def cyclic_gen(gen):
    for element in itertools.cycle(gen):
        yield element
...
def convolve(x, y, p, kernel_width):
    sq = torch.sum((x.unsqueeze(1) - y.unsqueeze(0)) ** 2, 2)
    return torch.mm(torch.exp(-1 * sq / (kernel_width ** 2)), p)
...
def compute_jacobian(f, inputs, create_graph=False):
    """Computes the jacobian of outputs with respect to inputs
    See : https://gist.github.com/sbarratt/37356c46ad1350d4c30aefbd488a4faa
    """
    inputs_ = inputs.squeeze()
    outputs = f(inputs)
    jac = outputs.new_zeros(outputs.size() + inputs_.size()
                            ).view((-1,) + inputs_.size())
    ...
    if create_graph:
        jac.requires_grad_()
    return jac.view(outputs.size() + inputs_.size())


def compute_volume(f, inputs, create_graph=False):
    z_dim = inputs.squeeze().shape[0]
    jac = compute_jacobian(f, inputs, create_graph)
    jac = jac.view(-1, z_dim)
    metric = torch.t(jac) @ jac
    return .5 * torch.sum(torch.log(torch.svd(metric)[1]))
